...
class MgrUI{
    //初始化UI
    public static welcomeUI:WelcomeUI = new WelcomeUI;       //开始游戏页面UI
    public static gameUI:GameUI = new GameUI;   //游戏页面
    public static endUI:EndUI = new EndUI;     //游戏完成页面
    public static statusUI:number = -1;//用来标志着当前是哪个UI页面：初始值定位-1,0代表欢迎页面，1代表游戏页面
    public static nextPinDist:number = 0;
    public static scoreText:Laya.Text = new Laya.Text;//分数
    constructor()
    {
    }
    //初始化欢迎页面
    public static startWelcomeUI():void{
        MgrUI.statusUI = 0;//欢迎UI
        Laya.stage.removeChildren();//移除了所有的ui
        MgrUI.welcomeUI.InitUI();
    }
    //初始化游戏页面
    public static startGameUI():void{
    ...
    //初始化静态数据
    public static initStaticData():void{
    ...
    //重置静态数据
    public static resetStaticData():void{
        //数据清空
		ConstPara.balloons = [];
		ConstPara.pins = [];
		ConstPara.bonuses = [];
    ...
    //注册监听事件
    public static addEventListenings():void{
    ...
    //移除监听事件
    public static removeEventListenings():void{
        //Laya.timer.frameLoop(1,this,MgrUI.UpdateFrames);//每帧执行一次
    ...
    //开始点击
    public static touchesBegan(e:Event):void {
        //console.log("监听鼠标按下"); 
		var touchLocation:Laya.Point = WYPoint.make(Laya.stage.mouseX, Laya.stage.mouseY);//生成鼠标点
    ...
    //移除点击
    public static touchesEnded(e:Event) :void{
        //console.log("监听鼠标抬起"); 
		ConstPara.isMousePressed = false;
	}
    public static touchesMoved(e:Event) :void{
		if (ConstPara.isMousePressed) {
            //console.log("监听鼠标移动"); 
			var touchLocation:Laya.Point = WYPoint.make(Laya.stage.mouseX, Laya.stage.mouseY);
			ConstPara.mouse = touchLocation;
		}
	}

    //注册全部update事件
    public static UpdateFrames():void{
		...
		if (!ConstPara.isGameOver) {
			var dlt:number = ConstPara.pinMaxY - ConstPara.LOW_V_AREA;
            //快速下降
			if (dlt < 0) {
    ...
    private static scrollData(y:number):void {
        //y = 0.15;
		...
		for (var k2 in ConstPara.pins) {
            var p:Pin = ConstPara.pins[k2];
			p.pos.y += y;
		}
		for (var k3 in ConstPara.bonuses) {
            var b1:Bonus = ConstPara.bonuses[k3];
			b1.pos.y += y;
		}
...
//					- Math.abs(nextPinDist), ScriptLayer.this));
    ...
    // }
    //显示分数
    public static getNumOfBonus():void{
    ...
    //更新每个actor
    public static updateActors(actors:Array<any>){
        var actors_size:number = actors.length;//计算数组长度
		var actor;
		for (var i:number = actors_size - 1; i > -1; i--) {
			actor = actors[i];
			if (actor.update()) {
