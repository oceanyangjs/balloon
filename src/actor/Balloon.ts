...
class Balloon extends Actor{
    public constructor(p: Laya.Point) {
    	super();
        this.pos = p;
        this.size = Balloon.BASE_SIZE / 2;
		...
		this.sprite.graphics.drawCircle(this.sprite.x,this.sprite.y,Pin.SIZE,"#00FF7F");
    ...
    public isBlowingUp:boolean = true;//是否是膨胀状态
    public vel:Laya.Point = new Laya.Point;//这个代表什么呢--似乎是最下面的点--代表方向向量--目前当做向量坐标--更新：此为方向的向量坐标
    public static BASE_SIZE:number = 20;//基础大小
    public MOST_LARGE_SIZE:number = 20 * 6;//最大大小
...
//			vel.y += 1f;
		...
		for (var  k in ConstPara.pins) {//遍历一下那些障碍物
            var p:Actor = ConstPara.pins[k] as Actor;
		...
		for (var k in ConstPara.balloons) {//遍历一下气球
			var b:Balloon = ConstPara.balloons[k] as Balloon;
		...
		var scaleNum:number = this.size / Balloon.BASE_SIZE;//图片缩放
