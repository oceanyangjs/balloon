...
class Bonus extends Actor{
    constructor(y:number = 0){
        ...
        if (y == 0) 
            this.pos.x = Util.randw(0.1, 0.8);
        else 
            this.pos.x = Util.randw(0.5, 0.2);
        //this.sprite.loadImage("../../laya/assets/comp/bonus.png",0,0);
        this.pos.y = y - ConstPara.SIZE / 2;//--y==640 ConstPara.SIZE=16
        this.pos.y = 10;
        this.sprite.alpha = 0;
        this.sprite.graphics.drawRect(this.sprite.x,this.sprite.y,ConstPara.SIZE,ConstPara.SIZE,"#ff0000");
        ...
        for (var k in ConstPara.pins) {
            ...
            if(WYPoint.distance(p.pos,this.pos) < ConstPara.SIZE*3)
                p.isRemoving = true;
        }
    }
    //每帧刷新--应该是需要调用
